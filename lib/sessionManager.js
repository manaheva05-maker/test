...
export default class SessionManager extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this._initMongoStore();
  }
  
  async _initMongoStore() {
    try {
      const mongoModule = await import('./mongoStore.js');
      this.mongoStore = mongoModule.default;
      await this.mongoStore.connect();
    } catch (e) {
      console.warn("MongoDB store not available:", e?.message || e);
    }
  }
  ...
  async startAll() {
    ...
    // Restore sessions from MongoDB if available
    if (this.mongoStore && this.mongoStore.connected) {
      try {
        const savedSessions = await this.mongoStore.getAllSessions();
        console.log(`Found ${savedSessions.length} saved sessions in MongoDB`);
        
        for (const saved of savedSessions) {
          if (saved.sessionId && !this.sessions.has(saved.sessionId)) {
            this.register(saved.sessionId);
            console.log(`Restored session ${saved.sessionId} from MongoDB`);
          }
        }
      } catch (error) {
        console.warn("Failed to restore sessions from MongoDB:", error.message);
      }
    }
  ...
  async logout(sessionId) {
    ...
    if (this.mongoStore && this.mongoStore.connected) {
      try {
        await this.mongoStore.deleteSession(sessionId);
      } catch (e) {
        console.warn("Failed to delete session from MongoDB:", e?.message || e);
      }
    }
